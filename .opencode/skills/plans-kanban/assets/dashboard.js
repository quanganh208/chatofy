...
(function () {
  ...
  function applyFiltersAndSort() {
    ...
    if (state.search) {
      filtered = filtered.filter(
        (p) =>
          p.name.toLowerCase().includes(state.search) || p.id.toLowerCase().includes(state.search),
      );
    }
  ...
  function renderKanbanCard(plan) {
    ...
    if (plan.description) {
      const desc =
        plan.description.length > 80 ? plan.description.slice(0, 77) + '...' : plan.description;
