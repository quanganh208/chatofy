/**
 * Hand-rolled minimal YAML subset parser for `.agentkit/config.yaml`,
 * shared (by copy, not by reference — this skill has no dependency on
 * ak-journal) with `kits/core/skills/ak-journal/scripts/mini-yaml-parser.cjs`.
 *
 * Supports exactly what the `worktree:` block of `.agentkit/config.yaml` /
 * `~/.agentkit/config.yaml` needs:
