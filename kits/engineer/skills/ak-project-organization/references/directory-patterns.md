...
Evergreen current project documentation serves humans and AI agents. Preserve the repository's existing documentation root, navigation, naming, and ownership conventions. Do not create or rename a standard corpus merely to match this skill.

**Optional example only** — adapt these purpose-based placeholders when a project has no stronger convention:

```text
docs/
├── <product-or-domain-overview>.md   # Goals, users, and requirements
├── <architecture-or-source-map>.md   # Components and ownership
├── <development-conventions>.md      # Coding and review practices
├── <operations-guide>.md             # Setup, deployment, or recovery
...
- Follow existing naming and date conventions before applying the optional example.
- Keep one authority for each durable fact and link to it instead of copying inventories.
- Split documents only when it improves navigation or follows a project-defined limit.

## Plans (`plans/`)

Implementation plans, research, agent communication, and chronological work
records.
...
├── journals/                        # Historical session/event records
│   └── {YYMMDD-HHmm}-{slug}.md
...
- Journals are timestamped work history, not current docs or decision authority
