...
> **Own-brand vs. referenced-brand**: `brand` skill governs _your own_ brand system (define, audit, evolve). When the current task instead references someone else's brand (client, competitor, product being illustrated), load `references/brand-asset-protocol.md` — that protocol governs _acquiring_ real assets (logo, product image, UI) before generating, and it is a hard stop when a real logo cannot be located.
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/skills/ak-brand/scripts/inject-brand-context.cjs
node ${CLAUDE_PLUGIN_ROOT}/skills/ak-brand/scripts/validate-asset.cjs <path>
```

**Tokens:**

```bash
node ${CLAUDE_PLUGIN_ROOT}/skills/ak-design-system/scripts/generate-tokens.cjs -c tokens.json
node ${CLAUDE_PLUGIN_ROOT}/skills/ak-design-system/scripts/validate-tokens.cjs -d src/
```
