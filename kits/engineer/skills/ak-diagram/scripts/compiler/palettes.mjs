...
export function roleTokenCss() {
  ...
  const perFamily = Object.keys(ROLE_COLORS)
    .map(
      (family) =>
        `.ak-node[data-family="${family}"]{--ak-node-stroke:var(--ak-role-${family}-stroke);--ak-node-fill:var(--ak-role-${family}-fill);}` +
        `.ak-legend-item[data-family="${family}"] .ak-legend-swatch{fill:var(--ak-role-${family}-fill);stroke:var(--ak-role-${family}-stroke);}`,
    )
    .join('\n');
  return [
...
export function paletteCss(preset, theme, selector = 'svg') {
  const palette =
    PRESET_THEME_PALETTES[`${preset}:${theme}`] || PRESET_THEME_PALETTES['classic:light'];
