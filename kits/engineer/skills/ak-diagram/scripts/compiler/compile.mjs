...
function layoutWorkflow(data) {
  ...
  return {
    ...
    lanes: renderedLanes,
    boundaries: [],
    totalWidth: Math.max(maxW + 40, 700),
    totalHeight: Math.max(currentY + 40, 400),
    components: steps,
  };
}
...
function layoutDataflow(data) {
  ...
  nodes.forEach((n) => {
    if (n.stage && stageMap.has(n.stage)) {
      ...
      if (!stageMap.has('default'))
        stageMap.set('default', { id: 'default', label: 'Processing', nodes: [] });
  ...
  return {
    ...
    boundaries: [],
    totalWidth: Math.max(startX + 40, 700),
    totalHeight: Math.max(maxH + 80, 400),
    components: nodes,
  };
}
...
function layoutLifecycle(data) {
  ...
  return {
    ...
    boundaries: [],
    totalWidth: Math.max(maxW + 40, 700),
    totalHeight: Math.max(currentY + stateH + 80, 400),
    components: states,
  };
}
...
function compile(rawJson, options = {}) {
  ...
  const subtitleHtml =
    data.meta && data.meta.subtitle
      ? `<span class="ak-diagram-subtitle"> — ${escapeXml(data.meta.subtitle)}</span>`
      : '';
