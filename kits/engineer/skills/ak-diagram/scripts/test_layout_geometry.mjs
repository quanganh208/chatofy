...
const fixtures = readdirSync(FIXTURES_DIR).filter(
  (f) => f.endsWith('.json') && !f.startsWith('adversarial'),
);
...
for (const file of fixtures) {
  ...
  const inside = (x, y) =>
    x >= vb.x - 0.5 && x <= vb.x + vb.w + 0.5 && y >= vb.y - 0.5 && y <= vb.y + vb.h + 0.5;
  ...
  for (let i = 0; i < scene.nodes.length; i += 1) {
    ...
    if (
      !inside(scene.nodes[i].x, scene.nodes[i].y) ||
      !inside(scene.nodes[i].x + scene.nodes[i].w, scene.nodes[i].y + scene.nodes[i].h)
    ) {
  ...
  for (const edge of scene.edges) {
    for (const p of edge.points) {
      if (!inside(p.x, p.y))
        throw new Error(`${file}: edge ${edge.id} point (${p.x},${p.y}) leaves the viewBox`);
    }
    if (edge.labelBox) {
    ...
    for (let i = 0; i < edge.points.length - 1; i += 1) {
      for (const node of scene.nodes) {
        ...
        if (segmentHitsRect(edge.points[i], edge.points[i + 1], node)) {
          throw new Error(
            `${file}: edge ${edge.id} segment ${i} crosses unrelated node ${node.id}`,
          );
        }
      }
    }
  ...
  if (steps.length !== scene.nodes.length + scene.edges.length)
    throw new Error(`${file}: every node and edge must carry --step`);
  if (Math.max(...steps) > 40) throw new Error(`${file}: --step exceeds the finite motion budget`);
  if ((svg.match(/pathLength="100"/g) || []).length < scene.edges.length)
    throw new Error(`${file}: edge paths need pathLength for draw motion`);
  if (data.meta.animation === 'trace' && !svg.includes('ak-edge-flow'))
    throw new Error(`${file}: trace animation needs flow overlays`);
  if (!svg.includes('class="ak-legend"')) throw new Error(`${file}: legend missing`);
  if (!svg.includes('@keyframes ak-node-in'))
    throw new Error(`${file}: standalone svg must inline motion styles`);
  checks += 1;
  console.log(
    `✓ ${file}: ${scene.nodes.length} nodes, ${scene.edges.length} edges, viewBox ${Math.round(vb.w)}x${Math.round(vb.h)}`,
  );
}
