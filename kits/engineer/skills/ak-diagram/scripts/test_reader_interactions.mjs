...
class El {
  ...
  dispatch(t, ev = {}) {
    (this.listeners[t] || []).forEach((fn) =>
      fn({ stopPropagation() {}, preventDefault() {}, target: this, ...ev }),
    );
  }
  click() {
    this.dispatch('click');
  ...
  querySelector(sel) {
    return this._match(sel)[0] || null;
  }
  querySelectorAll(sel) {
    return this._match(sel);
  }
  _match(sel) {
...
function parseFragment(html) {
  ...
  root.setAttribute(
    'data-instance-id',
    (html.match(/data-instance-id="([a-f0-9]+)"/) || [])[1] || 'x',
  );
  ...
  viewport.classList.add('ak-diagram-viewport');
  root.append(viewport);
  const svg = new El('svg');
  svg.classList.add('ak-diagram-svg');
  svg.innerHTML = '';
  viewport.append(svg);

  const search = new El('input');
  search.classList.add('ak-diagram-search-input');
  root.append(search);
  ...
  const chapLabel = new El('span');
  chapLabel.classList.add('ak-chapter-label');
  root.append(chapLabel);
  const dataScript = new El('script');
  dataScript.classList.add('ak-diagram-data');
  dataScript.setAttribute('type', 'application/json');
  dataScript.textContent =
    (html.match(/<script type="application\/json" class="ak-diagram-data">([\s\S]*?)<\/script>/) ||
      [])[1] || '{}';
  root.append(dataScript);

  // Nodes
  const nodeRe =
    /<g class="ak-node" data-node-id="([^"]+)" data-label="([^"]*)" data-role="([^"]*)" data-desc="([^"]*)"/g;
  let m;
  while ((m = nodeRe.exec(html))) {
    const n = new El('g');
    n.classList.add('ak-node');
    n.setAttribute('data-node-id', m[1]);
    n.setAttribute('data-label', m[2]);
    n.setAttribute('data-role', m[3]);
    n.setAttribute('data-desc', m[4]);
    svg.append(n);
  }
  // Edges
  ...
  while ((m = edgeRe.exec(html))) {
    const e = new El('g');
    e.classList.add('ak-edge');
    e.setAttribute('data-from', m[1]);
    e.setAttribute('data-to', m[2]);
    e.setAttribute('data-edge-id', m[3]);
    svg.append(e);
  }
  return root;
}
...
globalThis.document = {
  ...
  addEventListener(t, fn) {
    if (t === 'keydown') docKeyHandlers.push(fn);
  },
  querySelectorAll() {
    return [];
  },
...
function countDimmed(root) {
  return root.querySelectorAll('.ak-node').filter((n) => n.classList.contains('is-dimmed')).length;
}
function countFocused(root) {
  return root.querySelectorAll('.ak-node').filter((n) => n.classList.contains('is-focused')).length;
}
function countHiEdges(root) {
  return root.querySelectorAll('.ak-edge').filter((e) => e.classList.contains('is-highlighted'))
    .length;
}
...
const archRoot = parseFragment(
  compile(readFileSync(join(FIXTURES, 'architecture-sample.json'), 'utf-8'), {
    format: 'fragment',
  }),
);
init(archRoot);
...
const gw = archRoot
  .querySelectorAll('.ak-node')
  .find((n) => n.getAttribute('data-node-id') === 'gateway');
gw.dispatch('click');
if (countFocused(archRoot) !== 1) throw new Error('focus: expected exactly 1 focused node');
if (countHiEdges(archRoot) < 1)
  throw new Error('focus: expected highlighted edges from focused node');
...
const authFocused = archRoot
  .querySelectorAll('.ak-node')
  .some((n) => n.getAttribute('data-node-id') === 'auth' && n.classList.contains('is-focused'));
...
if (countDimmed(archRoot) !== 0)
  throw new Error('reach-down: expected all nodes reachable from gateway');
...
docKeyHandlers.forEach((fn) => fn({ key: 'Escape', preventDefault() {}, target: {} }));
if (countFocused(archRoot) !== 0 || countDimmed(archRoot) !== 0)
  throw new Error('escape: expected cleared highlights');
...
const varUsages = Array.from(
  lastExportedBlobContent.matchAll(/var\((--ak-[a-z0-9-]+)\)/gi),
  (m) => m[1],
);
const uniqueUsages = new Set(varUsages);
const definedVars = new Set(
  Array.from(lastExportedBlobContent.matchAll(/(--ak-[a-z0-9-]+)\s*:/gi), (m) => m[1]),
);
const missingDefs = [...uniqueUsages].filter((v) => !definedVars.has(v));
if (missingDefs.length > 0) {
  throw new Error(
    `share: missing CSS variable definitions in exported SVG: ${missingDefs.join(', ')}`,
  );
}

// 2. Non-classic preset/theme verification (signal-flow:dark)
const sfRoot = parseFragment(
  compile(readFileSync(join(FIXTURES, 'architecture-sample.json'), 'utf-8'), {
    format: 'fragment',
    preset: 'signal-flow',
    theme: 'dark',
...
if (
  !lastExportedBlobContent.includes('--ak-bg: #031c18;') ||
  !lastExportedBlobContent.includes('--ak-accent: #34d399;')
) {
  throw new Error(
    'share: non-classic preset (signal-flow:dark) did not serialize accurate palette',
  );
}

// Check signal-flow reference-minus-definition completeness
const sfUsages = new Set(
  Array.from(lastExportedBlobContent.matchAll(/var\((--ak-[a-z0-9-]+)\)/gi), (m) => m[1]),
);
const sfDefs = new Set(
  Array.from(lastExportedBlobContent.matchAll(/(--ak-[a-z0-9-]+)\s*:/gi), (m) => m[1]),
);
...
console.log(
  '✓ Share card export serializes computed/fallback palette with reference-minus-definition completeness (classic + signal-flow).',
);
