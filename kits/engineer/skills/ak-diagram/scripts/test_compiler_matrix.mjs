  ...
  for (const preset of presets) {
    for (const theme of themes) {
      ...
      if (!fragOut.includes('<svg') || fragOut.includes('<!DOCTYPE html>')) {
        throw new Error(
          `Failed to compile ${arch} with preset=${preset}, theme=${theme} (Fragment)`,
        );
      }
...
console.log(
  `✓ All ${matrixCount} matrix combinations compiled successfully in SVG, Fragment, and HTML modes!`,
);
...
try {
  const dangData = {
    ...
    diagram_type: 'architecture',
    meta: { title: 'Dangling' },
    components: [{ id: 'node1', label: 'Node 1' }],
    connections: [{ from: 'node1', to: 'nonexistent-node' }],
  };
...
const xssData = {
  ...
  components: [{ id: 'n1', label: 'Label <script>alert(2)</script>' }],
  connections: [],
};
const xssHtml = compile(xssData, { format: 'html' });
if (
  xssHtml.includes('<script>alert(1)</script>') ||
  xssHtml.includes('<script>alert(2)</script>')
) {
...
const evtDataScript =
  (evtHtml.match(/<script type="application\/json"[^>]*>([\s\S]*?)<\/script>/) || [])[1] || '';
const evtVisible = evtHtml.replace(/<script type="application\/json"[\s\S]*?<\/script>/g, '');
if (
  evtVisible.includes('<img') ||
  evtVisible.includes('onerror=alert(3)>') ||
  evtVisible.includes('onload="alert(1)"')
) {
...
for (const arch of archetypes) {
  ...
  const netRefs =
    /(src|href)\s*=\s*["']https?:\/\/|@import\s+url\(\s*["']?https?:\/\/|cdn\.|fonts\.googleapis|<script\s+src=/i;
...
{
  const fragA = compile(readFileSync(join(FIXTURES_DIR, 'architecture-sample.json'), 'utf-8'), {
    format: 'fragment',
  });
  const fragB = compile(readFileSync(join(FIXTURES_DIR, 'workflow-sample.json'), 'utf-8'), {
    format: 'fragment',
  });
...
console.log('\n=== Running Semantic Rendering Tests (boundaries/lanes/stages/subtitle) ===');
{
  ...
  if (!archHtml.includes('Prod VPC') || !archHtml.includes('ak-boundary'))
    throw new Error('Architecture boundary not rendered');
  if (!archHtml.includes('My Subtitle')) throw new Error('meta.subtitle not rendered');

  const wfHtml = compile(readFileSync(join(FIXTURES_DIR, 'workflow-sample.json'), 'utf-8'), {
    format: 'fragment',
  });
  if (!wfHtml.includes('ak-lane') || !wfHtml.includes('CI Runner'))
    throw new Error('Workflow lane label not rendered');

  const dfHtml = compile(readFileSync(join(FIXTURES_DIR, 'dataflow-sample.json'), 'utf-8'), {
    format: 'fragment',
  });
  if (!dfHtml.includes('ak-stage') || !dfHtml.includes('Ingestion'))
    throw new Error('Dataflow stage label not rendered');
}
