...
test('resolve defaults under AGENTKIT_HOME', async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'ak-home-'));
  try {
    const loaded = await loadPreferences({ env: { AGENTKIT_HOME: dir } });
