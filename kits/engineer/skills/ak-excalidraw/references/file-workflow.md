...
```bash
cd ${CLAUDE_PLUGIN_ROOT}/skills/ak-excalidraw/references
uv sync
...
```bash
cd ${CLAUDE_PLUGIN_ROOT}/skills/ak-excalidraw/references && uv run python render_excalidraw.py <path-to-file.excalidraw>
```
