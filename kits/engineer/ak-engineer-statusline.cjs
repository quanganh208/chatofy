...
function buildUsageWindows(cache) {
  ...
  const snap = [
    {
      label: '5h',
      percent: cache.snapshot?.fiveHourPercent,
      resetsAt: cache.data?.five_hour?.resets_at,
    },
    {
      label: 'wk',
      percent: cache.snapshot?.weekPercent,
      resetsAt: cache.data?.seven_day?.resets_at,
    },
  ]
...
async function main() {
  try {
    ...
    const sessionContext = createSessionStateContext({
      ...
      cwd:
        process['env'].CK_PROJECT_ROOT || data.workspace?.current_dir || data.cwd || process.cwd(),
      requireBinding: true,
    });
