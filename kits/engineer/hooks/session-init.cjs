...
try {
  ...
  async function main() {
    ...
    try {
      ...
      const displaySource = ['startup', 'resume', 'clear', 'compact'].includes(source)
        ? source
        : 'unknown';
      console.log(
        `Session ${displaySource}. ${safeDisplayValue(buildContextOutput(config, detections, resolved, staticEnv.gitRoot), 4096)}`,
      );
      ...
      if (source === 'compact') {
        ...
        console.log(
          `Before continuing, reconcile the background processes you started earlier this`,
        );
        console.log(
          `session (dev servers, watchers, tunnels). Note the still-needed ones (command,`,
        );
        console.log(
          `PID, port, worktree) and stop the rest so orphaned processes do not pile up and`,
        );
        ...
        if (recovery) {
          ...
          if (recovery.mainRoot)
            console.log(`  Root project: ${safeDisplayValue(recovery.mainRoot)}`);
          if (recovery.branch)
            console.log(
              `  Branch: ${safeDisplayValue(recovery.branch)}${recovery.head ? ` @ ${safeDisplayValue(recovery.head)}` : ''}${recovery.dirtyCount ? ` (${recovery.dirtyCount} uncommitted)` : ''}`,
            );
          if (recovery.activePlan)
            console.log(`  Active plan: ${safeDisplayValue(recovery.activePlan)}`);
        }
        console.log(
          `Re-establish before continuing: the issues/PRs in flight, the active plan and`,
        );
        console.log(
          `current phase, what is done vs. still pending, any failures and their cause, and`,
        );
        console.log(
          `why the current approach was chosen. Re-read the active plan and notes first.`,
        );
      }
