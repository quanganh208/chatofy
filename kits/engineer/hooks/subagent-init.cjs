...
try {
  ...
  async function main() {
    ...
    try {
      ...
      const plansPath = toDisplayPath(
        path.join(baseDir, normalizePath(config.paths?.plans) || 'plans'),
      );
      const docsPath = toDisplayPath(
        path.join(baseDir, normalizePath(config.paths?.docs) || 'docs'),
      );
      ...
      lines.push(
        `- KISS / DRY. Full requested scope, nothing extra. YAGNI only if the delegated task explicitly passes --yagni (this rules line does not count)`,
      );
      ...
      lines.push(
        `- Report: ${toDisplayPath(path.join(reportsPath, `${agentType}-${namePattern}.md`))}`,
      );
