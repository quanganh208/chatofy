...
- [ ] `import('@chatofy/ui')` không kéo react/radix; output tokens byte-identical **sau install sạch**
- [ ] `token-parity.spec.ts` đỏ khi giá trị trong CSS entry của popup lệch — mutation-verified
- [ ] `theme.css` của popup **không** chứa `--font-sans` hay `@layer base`
- [ ] `pnpm install --frozen-lockfile` trên worktree sạch chạy được
- [ ] `packages/ui/README.md` không còn khẳng định "No dependencies" như luật hiện hành
- [ ] mobile typecheck + `expo export` không đổi
- [ ] Quyết định umbrella-vs-lẻ được ghi lại kèm lý do
