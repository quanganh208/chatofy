...
| 2   | [Phase 2: Channel delta gate](./phase-02-channel-delta-gate.md)                                             | **Harness landed 2026-09-01** — gate armed; awaiting the one-shot session           |
...
| 8   | [Phase 8: Speaker step off the audio path](./phase-08-speaker-step-off-the-audio-path.md)                   | **Withdrawn** — no budget to free; see Corrections above                            |
...
P8 — WITHDRAWN 2026-09-01 (correction C3), dropped from the chain
```

P2 declares `dependencies: [1, 7]`; P7 declares `[1]`. P5
...
11. **Provenance.** `SPEAKER_BENCH_REQUIRE_PARITY=1` is read in exactly one
    ...
    calling itself a hard gate.~~ **RESOLVED 2026-09-01, harness landed.** The
    script now exits `0` PASS/PROCEED, `1` STOP, `2` UNMEASURED (absent fixture
    included) — see Phase 2 Success Criteria. Verified: host bench
    `SPEAKER_BENCH_REQUIRE_PARITY=1 uv run pytest -q` 363 passed, exit 0.
...
| 6   | `SPEAKER_BENCH_REQUIRE_PARITY=1` is read only by pytest; `run_channel_delta.py` always exits 0                                                                                                        | Critical | **RESOLVED 2026-09-01** — harness landed, exit contract is now `0` PASS/PROCEED, `1` STOP, `2` UNMEASURED (Phase 2)                         |
...
| 14  | The recorder commits device fingerprints and timestamps — `.gitignore` re-includes `fixtures/**/*.json`; the retention decision cannot reach git history. **RESOLVED 2026-09-01** — `getSettings()` is whitelisted to four fields so `deviceId`/`groupId` never reach the log, `recordedAt` is a date-only `recordedOn`, and the retention date is required before the microphone opens and written into the log naming git history. A fourth location the finding did not know about — the new per-turn CSV, under a tracked `results/` — is gitignored (Phase 2) | High     | Accept      | Phase 2                |
| 15  | `COUNT_ERROR_MAX` also constrains `calibrate()`; the script's gate cell and exit code still point at warm N=5; P2's 5s bucket raises `KeyError` and the verdict is hardcoded to bucket 2.0. **Gate-bucket half RESOLVED 2026-09-01** — `GATE_BUCKET_S` is now a parameter set from M1 (`1.0`, `--gate-bucket`), no silent fallback; a bucket nothing scored exits UNMEASURED (Phase 2)                                                                                                                                                                             | High     | Accept      | Phase 1, Phase 2       |
