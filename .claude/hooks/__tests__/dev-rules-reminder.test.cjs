    ...
    it('includes absolute paths in Reports output', async () => {
      ...
      if (result.stdout) {
        const hasAbsolutePath =
          result.stdout.includes(gitRoot) || result.stdout.match(/Reports:.*\//);
  ...
  describe('Transcript Deduplication', () => {
    ...
    it('deduplicates repeated injections when transcript_path is null but session_id is present', async () => {
      ...
      try {
        ...
        assert.ok(
          firstRun.stdout.includes('[IMPORTANT] Consider Modularization'),
        ...
        assert.ok(
          fs.existsSync(sessionStatePath),
        ...
        assert.ok(
          Object.values(scopes).some((scope) => scope?.lastInjectedAt),
    ...
    it('reinjects when the same session moves to a different cwd without transcript_path', async () => {
      ...
      try {
        ...
        assert.ok(
          secondRun.stdout.includes('[IMPORTANT] Consider Modularization'),
          'Different cwd should inject refreshed reminder content',
        );
        assert.ok(
          secondRun.stdout.includes(tempDirB),
