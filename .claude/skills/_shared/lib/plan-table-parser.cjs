...
function parseFormat0(content, dir, options) {
  ...
  while (i < lines.length) {
    ...
    while (j < lines.length && lines[j].trim().startsWith('|')) {
      ...
      phases.push(
        makePhase(idMatch[1], idMatch[2], name, statusRaw, filePath, linkText, dir, options),
      );
      j++;
    }
...
function parsePlanPhases(content, dir, options = {}) {
  ...
  const f1Regex =
    /\|\s*(\d+)([a-z]?)\s*\|\s*([^|]+)\s*\|\s*([^|]+)\s*\|\s*\[([^\]]+)\]\(([^)]+)\)/gi;
  while ((match = f1Regex.exec(content)) !== null) {
    phases.push(
      makePhase(match[1], match[2], match[3], match[4], match[6], match[5], dir, options),
    );
  }
  ...
  while ((match = f2bRegex.exec(content)) !== null) {
    phases.push(
      makePhase(match[1], match[2], match[3], match[5], match[4], match[3], dir, options),
    );
  }
  if (phases.length > 0) return phases;
  ...
  if (/^-\s*Phase\s*\d+[:\s]/m.test(content)) {
    ...
    for (const line of lines4) {
      ...
      if (cur4) {
        ...
        if (sm)
          cur4.status = sm[1].toLowerCase() === 'completed' ? 'completed' : normalizeStatus(sm[2]);
        if (
          /^##/.test(line) ||
  ...
  if (phaseMap.size > 0) {
    ...
    phases.push(
      ...Array.from(phaseMap.values()).sort(
        (a, b) => a.phase - b.phase || a.phaseId.localeCompare(b.phaseId),
      ),
    );
    return phases;
  }
