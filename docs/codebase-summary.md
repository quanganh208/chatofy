...
| `MinutesStore` (`MINUTES_STORE`)      | `apps/api/src/modules/minutes/interfaces/minutes-store.interface.ts`   | `MemoryMinutesStore` (default) or `PrismaMinutesStore`, selected by `MINUTES_STORE_BACKEND`                                                                              |
| `GlossaryStore` (`GLOSSARY_STORE`)    | `apps/api/src/modules/glossary/interfaces/glossary-store.interface.ts` | `MemoryGlossaryStore` (default) or `PrismaGlossaryStore`, selected by `GLOSSARY_STORE_BACKEND` — per-user domain-term glossary merged into translation hints server-side |
