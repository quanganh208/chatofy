...
- **Linger must stay enabled** (`loginctl enable-linger`). Docker Desktop is a _user_ service: without linger it starts on login, not on boot, so after an unattended reboot the runner would come up while the daemon did not — and `restart: unless-stopped` cannot help, because it is honoured by a daemon that is not running.
- **Model weights** — `~/chatofy/models/{stt,tts}`, mounted read-only.
